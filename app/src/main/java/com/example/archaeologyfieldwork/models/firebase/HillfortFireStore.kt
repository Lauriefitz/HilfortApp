package com.example.archaeologyfieldwork.models.firebase
...
class HillfortFireStore (val context: Context) : hillfortStore, AnkoLogger {
    ...
    override fun update(hillfort: MainModel) {
        val foundHillfort: MainModel? = hillforts.find { p -> p.fbId == hillfort.fbId }
    ...
    fun fetchHillforts(hillfortsReady: () -> Unit) {
        val valueEventListener = object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                dataSnapshot!!.children.mapNotNullTo(hillforts) { it.getValue<MainModel>(MainModel::class.java) }
                hillfortsReady()
            }
        }
    ...
    fun updateImage(hillfort: MainModel) {
        if (hillfort.image != "") {
